...
- 密码修改功能（旧密码验证 → 新密码 → 确认）
- WiFi 配网（AP 热点模式，手机配置）
- 低功耗睡眠（30 秒无操作自动休眠，按键唤醒）
...
### 密码修改
- 3 步流程：验证旧密码 → 输入新密码 → 确认新密码
- 最少 4 位，两次输入不一致会提示重试
- 修改成功后通过 WiFi 上报事件

### WiFi 配网
- ESP8266 切换 AP 模式，创建热点 "SmartLock"
- 手机连接热点后通过浏览器配置 WiFi 信息
- 配置完成后自动切回 Station 模式连接路由器

### 低功耗
- 30 秒无操作自动进入 Sleep 模式
- CPU 执行 WFI 指令暂停，外设继续运行
- 按键或指纹触摸通过中断唤醒，无需重新初始化
...
- [x] 密码修改功能（3 步流程：验证旧密码 → 输入新密码 → 确认）
- [x] WiFi 配网功能（AP 模式热点配置）
- [x] 低功耗待机模式（Sleep 模式，WFI 指令，中断唤醒）
- [ ] PCB 原理图上传
